...
def testimg(imgname, context):#根据context检测库中是否存在imgname.png图片
    ...
    for i in range(image_num):
        ...
        dif1 = (enc_v1.dot(enc_v1)).decrypt()
        dif2 = (enc_test.dot(enc_test)).decrypt()
        differ = (enc_v1.dot(enc_test)).decrypt()
        anc = math.sqrt(dif1[0]*dif2[0])
        mate = 100 * differ[0] / anc
        print("匹配度："+str(mate)+"%")
        # 比较相减结果的内积是否为0，是则匹配
        if (mate > max):
            index = i
            max = mate
    if(max > 99.999):
        print("test-image matches:"+str(index))
    else:
